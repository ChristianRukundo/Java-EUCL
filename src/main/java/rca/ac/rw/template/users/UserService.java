...
import rca.ac.rw.template.meter.MeterConverter; 
import rca.ac.rw.template.meter.MeterRepository;
import rca.ac.rw.template.token.PurchasedTokenConverter; 
...
public class UserService {
    ...
    public void createAdminUserIfNotExists() {
        if (userRepository.findByEmail(adminEmail).isEmpty()) {
            ...
            adminUser.setNationalId("0000000000000000"); 
            adminUser.setEmail(adminEmail);
            adminUser.setPassword(passwordEncoder.encode(adminPassword));
            adminUser.setRole(Role.ROLE_ADMIN);
            adminUser.setAddress(new Address("Rwanda", "Nyamagabe", "Kaganza"));
     ...
     */
    /**
     * Retrieves the profile of the currently authenticated user, including their
     * registered meters and any NEW (unused) electricity tokens.
     *
     * @return UserProfileResponseDto containing the comprehensive profile details.
     */
    @Transactional(readOnly = true)
    public UserProfileResponseDto getMyProfile() {
        User authenticatedUser = getAuthenticatedUser();
        log.debug("Fetching profile details for user: {}", authenticatedUser.getEmail());


        UserProfileResponseDto profileDto = UserConverter.toUserProfileResponseDtoBasic(authenticatedUser);


        if (authenticatedUser.getRole() == Role.ROLE_CUSTOMER) {
            List<Meter> userMeters = meterRepository.findByUserAndDeletedFalse(authenticatedUser);
            if (!userMeters.isEmpty()) {
                profileDto.setMeters(userMeters.stream()
                        .map(MeterConverter::toDto)
                        .collect(Collectors.toList()));
                log.debug("Found {} meters for user {}", userMeters.size(), authenticatedUser.getEmail());


                List<TokenSummaryResponseDto> allNewTokens = userMeters.stream()
                        .flatMap(meter -> purchasedTokenRepository
                                .findByMeterAndTokenStatus(meter, TokenStatus.NEW)
                                .stream())
                        .map(PurchasedTokenConverter::toTokenSummaryResponseDto)
                        .collect(Collectors.toList());

                if (!allNewTokens.isEmpty()) {
                    profileDto.setNewTokens(allNewTokens);
                    log.debug("Found {} NEW tokens across all meters for user {}", allNewTokens.size(), authenticatedUser.getEmail());
                } else {
                    profileDto.setNewTokens(Collections.emptyList());
                }
            } else {
                profileDto.setMeters(Collections.emptyList());
                profileDto.setNewTokens(Collections.emptyList()); 
                log.debug("No meters found for user {}", authenticatedUser.getEmail());
            }
        } else {
            profileDto.setMeters(Collections.emptyList());
            profileDto.setNewTokens(Collections.emptyList());
    ...
    public User getAuthenticatedUser() {
        ...
        if (principal instanceof UUID) {
            ...
            log.debug("Fetching authenticated user by ID: {}", userId); 
